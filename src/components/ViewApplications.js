...
const ViewApplications = () => {
    ...
    const fetchData = async () => {
      try {
        const [applicationsRes, jobRes] = await Promise.all([
          axios.get(`${BASE_URL}/api/admin/postings/${id}/applications`),
          axios.get(`${BASE_URL}/api/postings/${id}`)
        ]);
  ...
  const handleShortlist = async () => {
    setLoading(true);
    try {
      const { data } = await axios.post(`${BASE_URL}/api/admin/postings/${id}/shortlist`, { count });
      setShortlisted(data);
      setError('');
    } catch (error) {
