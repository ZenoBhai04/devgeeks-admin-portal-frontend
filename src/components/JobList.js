...
const JobList = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const res = await axios.get(`${BASE_URL}/api/postings`);
        const data = res.data;

        // Normalize response: prefer an array response, fall back to .rows (pg style), otherwise empty
        if (Array.isArray(data)) {
          setJobs(data);
        } else if (data && Array.isArray(data.rows)) {
          setJobs(data.rows);
        } else {
          console.warn('/api/postings returned unexpected shape:', data);
          setJobs([]);
        }
      } catch (error) {
                    ...
                    >
                      <Box
                        sx={{
                          ...
                          borderRadius: "50%",
                          background:
                            "linear-gradient(135deg, rgba(57, 255, 20, 0.15), rgba(20, 255, 229, 0.15))",
                          border: "2px solid",
                          borderColor:
                            hoveredCard === job.id
                              ? "primary.main"
                      ...
                      >
                        <Business
                          sx={{ fontSize: 24, color: "primary.light" }}
                        />
                      </Box>
        ...
        <Fade in timeout={600}>
          ...
          >
            ...
            <Typography variant="body1" sx={{ mt: 4, color: "text.secondary" }}>
              Join our community and be the first to know when new positions are
              posted!
            </Typography>
          </Paper>
        </Fade>
